...
;(function (root, factory) {
  ...
  console.log('CryptoJS', _CryptoJS)
  CryptoJS = _CryptoJS
})
...
console.log('Add EvpKDF', CryptoJS)
;(function (root, factory, undef) {
  factory(CryptoJS)
})(this, function (CryptoJS) {
  /**
...
console.log('Add cipher-core', CryptoJS)
;(function (root, factory, undef) {
  factory(CryptoJS)
})(this, function (CryptoJS) {
  ;(function () {
...
function downloadImage(url, options, onComplete) {
  console.log('开始解析图片', url)
  // if (url.indexOf('6aa0aa6a-ebee') === -1) {
  //   downloadDomImage(url, options, onComplete)
  // } else {
  console.log('解析加密图片', url)
  const base64Data = decryptArrayBuffer(url)
  const index = url.lastIndexOf('.')
  const suffix = url.substring(index + 1)
  const typeStr = 'data:image/' + suffix + ';base64,'
  const data = typeStr + base64Data
  downloadDomImage(data, options, onComplete)
  // }
}

function decryptArrayBuffer(url) {
  const key = CryptoJS.enc.Utf8.parse('onukD&5ITJeDJB3jV^0ZTlyPgEfdrSBJ')
...
cc.assetManager.parser.register('.png', downloadImage)
